...
parser.add_argument('--tensorboard_path', default='./runs',
                    type=str, help='path to tensorboard files')
...
def train(epoch, net, net2, optimizer, labeled_trainloader, unlabeled_trainloader):
    ...
    for batch_idx, (inputs_x, inputs_x2, labels_x, w_x) in enumerate(labeled_trainloader):
        ...
        if (batch_idx + 1) % 100 == 0:
            writer.add_scalar("train/labeled_loss", Lx.item(),
                              (epoch * len(labeled_trainloader)) + batch_idx + 1)
            writer.add_scalar("train/unlabeled_loss", Lu.item(),
                              (epoch * len(unlabeled_trainloader)) + batch_idx + 1)
            writer.flush()
...
loader = dataloader.cifar_dataloader(args.dataset, r=args.r, noise_mode=args.noise_mode, batch_size=args.batch_size, num_workers=5,
                                     root_dir=args.data_path, log=stats_log, writer=writer, noise_file='%s/%.1f_%s.json' % (args.data_path, args.r, args.noise_mode))
